...
## 💡 Optionale Erweiterungsfunktionen (Blaupause)

> Diese Funktionen sind nicht zwingend für das MVP, machen den Use Case aber zu einer gut wiederverwendbaren Blaupause für spätere Ausbaustufen oder ähnliche Content-/Produkt-Onboarding-Szenarien.

### Erweiterter Architektur-Blick (funktionale Module)

```mermaid
flowchart LR
    subgraph INPUT[Input & Vorbereitung]
        WF[Watch-Folder<br/>Produktordner + READY-Datei]
        META[Metadaten / Fragebogen]
    end

    subgraph ORCH[Orchestrierung & Business-Logik]
        VALID[Validierung<br/>Struktur & Dateinamen]
        RULES[Regel-Engine<br/>(Taxonomie, Naming, SEO-Regeln)]
        QUEUE[Queue UC-003<br/>Transaction je Produkt]
    end

    subgraph AI[KI-Services]
        VISION[Vision-KI<br/>Farben, Muster, Stil]
        TEXT[Text-KI<br/>Produkttexte & optional SEO]
    end

    subgraph SHOP[Shop & Kanäle]
        WC[WooCommerce<br/>Produkt privat]
        CH[Weitere Kanäle<br/>Etsy, Pinterest, Instagram, Facebook]
    end

    subgraph HIL[Human-in-the-Loop]
        REVIEW[Review-UI<br/>UiPath Form / Power Apps]
        FEEDBACK[Optionale Feedback-Datenbank<br/>Korrekturen & Ratings]
    end
...
- **SEO- und Kanal-Optimierung**
  - Automatische Generierung von Meta-Titeln/-Descriptions und SEO-Keywords aus Vision-/Text-KI (abhängig von Entscheidung zu Q13).
  - Kanal-spezifische Textvarianten (z. B. verkürzte Titel/Teaser für Etsy/Instagram), gesteuert über die Regel-Engine.
- **Taxonomie- & Feedback-Loop**
  - Speicherung von Korrekturen aus dem Review-Formular (z. B. angepasste Muster-/Farb-Tags, überarbeitete Texte) in einer Feedback-Datenbank.
  - Nutzung dieser Daten zur iterativen Verbesserung von Prompts, Taxonomie und ggf. Modellwahl (spätere Phase, verknüpft mit Q15).
- **Self-Service-Produktstatus & Monitoring**
  - Einfaches Dashboard (z. B. UiPath Apps oder externes Reporting) mit Status je Produkt: "eingegangen", "in Verarbeitung", "wartet auf Review", "freigegeben", "live".
  - Filter-/Suchmöglichkeiten nach Kollektion, Designer, Status, Datum.
- **Mehrsprachigkeit & Lokalisierung**
  - Erweiterung der Text-KI-Aufrufe um parallele Generierung von EN-Texten (und später weiteren Sprachen), mit separaten Prompts/Guidelines.
  - Mapping je Sprache auf die entsprechenden WooCommerce-Felder oder Multilingual-Plugins.
- **Standardisierte Schnittstelle für weitere Use Cases**
  - Sauber definierte, generische Schnittstellen (z. B. JSON-Schema für Vision-/Text-Output), sodass andere Use Cases später dieselben KI-Bausteine und das gleiche Orchestrierungs-Pattern nutzen können.

Diese Erweiterungen müssen für UC-003 nicht alle im MVP umgesetzt werden, sollten aber bei Architektur-Entscheidungen und Naming/Taxonomie soweit möglich bereits mitgedacht werden, um spätere Anpassungen zu vereinfachen.
